...
import { Moon, Sun, Bell, Flame, Dumbbell, Scale, Activity, Timer, ClipboardList, Home, TrendingUp, User, ChevronRight, Settings, Target, Info } from 'lucide-react';
...
const HomeScreen = ({ isDark }) => (
    <div className="space-y-6 animate-fade-in">
        <section className="grid grid-cols-3 gap-2 px-5 mt-5">
            <div className={`${t(isDark, 'bg-white border-gray-100', 'bg-gray-800 border-gray-700')} p-3 rounded-2xl shadow-sm border text-center flex flex-col justify-center items-center`}>
            ...
            <div className={`${t(isDark, 'bg-white border-gray-100', 'bg-gray-800 border-gray-700')} p-3 rounded-2xl shadow-sm border text-center flex flex-col justify-center items-center`}>
                <Dumbbell size={20} className="mb-1 text-gray-500" />
            ...
            <div className={`${t(isDark, 'bg-white border-gray-100', 'bg-gray-800 border-gray-700')} p-3 rounded-2xl shadow-sm border text-center flex flex-col justify-center items-center`}>
                <Scale size={20} className="mb-1 text-blue-500" />
                ...
                <div className="bg-gray-900/40 backdrop-blur-sm rounded-[1.4rem] p-5 h-full border border-white/10 relative z-10">
                    ...
                    <div className="flex items-center justify-between mt-auto">
                        <div className="flex gap-3 text-[10px] font-bold text-white/80">
                            <span className="flex items-center gap-1"><Timer size={12} /> 55m</span>
                            <span className="flex items-center gap-1"><ClipboardList size={12} /> 6 Ex.</span>
                        </div>
...
const TrainingsScreen = ({ isDark }) => (
    <div className="px-5 mt-5 space-y-4 animate-fade-in">
        ...
        {[
            { title: "Treino A - Superiores", desc: "Força e Hipertrofia", icon: <Activity size={24} className="text-blue-500" />, tags: ["Avançado", "Última: Ontem"] },
            { title: "Treino B - Inferiores", desc: "Foco em Quadríceps", icon: <Target size={24} className="text-green-500" />, tags: ["Avançado", "Última: Há 3 dias"] },
            { title: "Cardio e Core", desc: "Recuperação Ativa", icon: <Flame size={24} className="text-orange-500" />, tags: ["Iniciante", "Última: Hoje"] }
        ].map((train, i) => (
            <div key={i} className={`${t(isDark, 'bg-white border-gray-100', 'bg-gray-800 border-gray-700')} p-4 rounded-2xl shadow-sm border flex gap-4 cursor-pointer active:scale-[0.98] transition-transform`}>
                <div className={`w-14 h-14 rounded-xl ${t(isDark, 'bg-gray-100', 'bg-gray-700')} flex items-center justify-center`}>{train.icon}</div>
...
const ProfileScreen = ({ isDark }) => (
    <div className="px-5 mt-5 space-y-6 animate-fade-in">
        <div className="flex flex-col items-center text-center mt-4">
            ...
            <p className="text-xs text-gray-500 font-bold mt-1">Software Dev & Criador do GymUp</p>
        </div>
        ...
        <div className={`${t(isDark, 'bg-white border-gray-100 divide-gray-100', 'bg-gray-800 border-gray-700 divide-gray-700/50')} rounded-3xl shadow-sm border overflow-hidden divide-y`}>
            {[
                { icon: <Settings size={18} className="text-gray-500" />, label: 'Configurações de Conta' },
                { icon: <Target size={18} className="text-gray-500" />, label: 'Ajustar Metas' },
                { icon: <Info size={18} className="text-gray-500" />, label: 'Sobre o GymUp' }
            ].map((item, i) => (
                <div key={i} className={`p-4 flex items-center justify-between cursor-pointer ${t(isDark, 'hover:bg-gray-50', 'hover:bg-gray-700/50')} transition-colors`}>
                    ...
                    <ChevronRight size={16} className={t(isDark, 'text-gray-300', 'text-gray-600')} />
                </div>
            ))}
        </div>
    </div>
        ...
        <div className="flex justify-center items-center w-full py-10 relative">
            <div className="absolute top-10 right-0 md:top-14 md:right-4 z-50 flex items-center gap-2">
                <button 
                    ...
                    className={`p-3 rounded-full shadow-xl border transition-all duration-300 hover:scale-110 ${isDarkMode ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-200 text-gray-900'}`}
                >
            ...
            <div className={`w-[340px] h-[680px] ${t(isDarkMode, 'bg-gray-50', 'bg-gray-900')} relative shadow-[0_20px_50px_rgba(0,0,0,0.3)] flex flex-col overflow-hidden border-[12px] border-gray-950 rounded-[3rem] transition-colors duration-500`}>
                ...
                <div className={`flex-1 overflow-y-auto pb-24 pt-6 transition-opacity duration-700 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                    {(activeTab === 'home' || activeTab === 'trainings') && (
                        <header className={`px-5 pt-4 pb-5 flex justify-between items-center ${t(isDarkMode, 'bg-white', 'bg-gray-800')} shadow-sm rounded-b-[1.5rem] relative z-10 transition-colors duration-300`}>
                            ...
                            <button aria-label="Notificações" className={`w-8 h-8 rounded-full ${t(isDarkMode, 'bg-gray-100 hover:bg-gray-200', 'bg-gray-700 hover:bg-gray-600')} flex items-center justify-center relative transition-colors`}>
                                <Bell size={16} className={t(isDarkMode, 'text-gray-600', 'text-gray-300')} />
                ...
                <div className={`absolute bottom-0 left-0 right-0 ${t(isDarkMode, 'bg-white/90 border-gray-200', 'bg-gray-900/90 border-gray-800')} backdrop-blur-xl border-t pb-2 z-50 rounded-b-[2.5rem] transition-colors duration-500`}>
                    <div className="flex justify-around items-center px-1 py-2">
                        <NavButton isDark={isDarkMode} icon={<Home size={20} />} label="Início" active={activeTab === 'home'} onClick={() => setActiveTab('home')} />
                        <NavButton isDark={isDarkMode} icon={<ClipboardList size={20} />} label="Treinos" active={activeTab === 'trainings'} onClick={() => setActiveTab('trainings')} />
                        <div className="relative -top-5">
                            <div className="w-12 h-12 bg-blue-600 rounded-full flex items-center justify-center text-white shadow-lg cursor-pointer hover:bg-blue-500 transition-transform active:scale-90">
                                <span className="text-2xl mt-[-2px]">+</span>
                            </div>
                        </div>
                        <NavButton isDark={isDarkMode} icon={<TrendingUp size={20} />} label="Evolução" active={activeTab === 'stats'} onClick={() => setActiveTab('stats')} />
                        <NavButton isDark={isDarkMode} icon={<User size={20} />} label="Perfil" active={activeTab === 'profile'} onClick={() => setActiveTab('profile')} />
                    </div>
